...
async def ensure_logged_in(page: Page, timeout_seconds: int = 60, on_timeout_callback=None) -> bool:
    """
    Navigate to LinkedIn login and wait for user to sign in.
    Checks every 2 seconds if login succeeded (looks for logged-in indicators).
    ...
    while asyncio.get_event_loop().time() - start_time < timeout_seconds:
        try:
            ...
            selectors = [
                ...
                "[data-test-id='global-nav-me-photo']",
                ".global-nav__button--profile"
            ]
